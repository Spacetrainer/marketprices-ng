...
--
--   1. is_tracked is FALSE for all 266 rows. False means "cataloged, not
...
--      NULL UNIQUE and the sheet carries no abbreviations, so the eighteen
--      below are derived from the unit names and are worth a second read before
--      they ship. They are display shorthand, nothing computes on them.
--
...
-- ----------------------------------------------------------------------------
-- units -- the 18 actually used as a RETAIL unit by the seeded products
--
-- Sixteen of the eighteen come from the source file; the Derica and Per cut do
-- not, and are noted below. Still absent are every unit that appears ONLY on
-- the wholesale side -- Carton (20 kg), 25-litre keg, Half cow, Lot of 10
-- birds, Bag of 100 and the rest. `units` has NO is_active column, so unlike
-- commodities and collection_sites a unit cannot be retired once it exists, and
-- generate-form-options.ts reads the whole table with no filter: every row here
-- is a choice offered to a collector. A unit nothing references is a wrong
-- answer put in front of someone pricing ugwu leaf. They arrive when there is a
-- wholesale price to record in them, or when `units` gains a way to be
-- filtered. 0040's header carries the wholesale pairing for all 65 of its rows,
-- so nothing is lost by leaving them out.
--
